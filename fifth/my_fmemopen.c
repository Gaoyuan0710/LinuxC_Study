 ...
 * =====================================================================================
 *
 *       Filename:  my_fmemopen.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  2014年07月17日 14时49分42秒
 *       Revision:  none
 *       Compiler:  gcc
 *
...
static int mstream_write(void *, char *, int);
...
FILE *fmemopen(void *__restrict buf, size_t size, const char *__restrict type)
{
	...
	if ((ms->flag = type_to_flags(type)) == 0)
	{
		errno = EINVAL;
		free(ms);
		return (NULL);
	...
	if (buf == NULL)
	{
		if ((ms->flag & (MS_READ | MS_WRITE)) != (MS_READ | MS_WRITE))
		{
			errno = EINVAL;
			free(ms);
			return(NULL);
...
static int type_to_flag(const char *__restrict type)
{
	const char *cp;
	int flags = 0;
...
static int find_end(char *buf, size_t len)
{
	off_t off = 0;
...
static int mstream_write(void *cookie, const char *buf, int len)
{
	...
	{
		ms->vsize = ms->curpos;
		if (((ms->flag & (MS_READ | MS_WRITE)) ==
						(MS_READ | MS_WRITE)) && (ms->vsize < 
								ms->rsize))
		{
			*(ms->buf + ms->vsize) = 0;
		}
	}
	if ((ms->flag & (MS_WRITE | MS_APPEND)) && !(ms->flag & MS_READ))
	{
...
struct fpos_t mstream_seek(void *cookie, fpos_t pos, int whence)
{
	int off;
	struct memstream *ms = cookie;
